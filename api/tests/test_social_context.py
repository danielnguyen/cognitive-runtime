...
def _seed_relationship(client: TestClient) -> str:
    client.post("/v1/relationships/entities/upsert", json={**_base(), "entity": _entity("project:alpha", label="project alpha", entity_type="project", domain="project_context")})
    client.post("/v1/relationships/entities/upsert", json={**_base(), "entity": _entity("repo:alpha", label="repo alpha", entity_type="repository", domain="project_context")})
...
def test_social_context_diagnostics_expose_suppression_and_references_without_hidden_scores():
    ...
    assert set(diagnostics["items"][0]["suppression_reasons"]) == {"low_confidence", "mentionability_restricted"}
